...
//			index, CMannVsMachineUpgradeManager::Upgrades().Count());
	...
	int GetUpgradePriority(const UpgradeInfo& info)
	{
		...
		if (info.pclass == TF_CLASS_SNIPER) {
			if (info.slot == TF_WPN_TYPE_PRIMARY &&
		...
		if (info.pclass == TF_CLASS_MEDIC) {
			if (info.slot == TF_WPN_TYPE_SECONDARY &&
		...
		if (info.pclass == TF_CLASS_SPY) {
			if (info.slot == TF_WPN_TYPE_MELEE &&
		...
		if (info.slot == TF_WPN_TYPE_ACTION) {
			return -10;
		}
	...
	void CTFBotPurchaseUpgrades::CollectUpgrades()
	{
		for (int slot : {
			-1,
			(int)TF_WPN_TYPE_PRIMARY,
			(int)TF_WPN_TYPE_SECONDARY,
			(int)TF_WPN_TYPE_MELEE,
			(int)TF_WPN_TYPE_BUILDING,
			(int)TF_WPN_TYPE_PDA,
		//	(int)TF_WPN_TYPE_ACTION,
		}) {
