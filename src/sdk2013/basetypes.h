...
#define _SIGSEGV_SDK2013_OVERRIDE__PUBLIC_TIER0_BASETYPES_H 1
// sigsegv modifications [WRT: Valve SDK2013 0d8dceea 20150909 / AlliedModders 0ef5d3d4 20171105]
// - make AlignValue constexpr
// - make Clamp, Min, Max constexpr
// - make IsFinite constexpr, using std::isfinite
// - make FloatMake(Negative|Positive) constexpr, using std::signbit
// - make FLOAT32_NAN constexpr-friendly
...
constexpr bool IsFinite( vec_t f )
{
...
constexpr float FloatMakeNegative( vec_t f )
{
	return std::signbit(f) ? -f : f;
}

constexpr float FloatMakePositive( vec_t f )
{
	return std::signbit(f) ? f : -f;
}

constexpr float FloatNegate( vec_t f )
{
	return -f;
}
...
constexpr bool color32::operator!=( const color32 &other ) const
{
