...
#define _SIGSEGV_SDK2013_OVERRIDE__PUBLIC_TIER0_PLATFORM_H 1
// sigsegv modifications [WRT: Valve SDK2013 0d8dceea 20150909 / AlliedModders 0ef5d3d4 20171105]
// - use static_assert for PLAT_COMPILE_TIME_ASSERT
// - remove '-Wc++11-extensions' warning suppression for override keyword
// - implement NORETURN for non-MSVC compilers
// - make fsel constexpr
// - make WordSwapC/DWordSwapC/QWordSwapC constexpr
// - make LoadLittleDWord/StoreLittleDword constexpr
// - replace multi-arg versions of Construct with a single variadic template function
...
static constexpr FORCEINLINE float fsel(float fComparand, float fValGE, float fLT)
{
	return fComparand >= 0 ? fValGE : fLT;
}
static constexpr FORCEINLINE double fsel(double fComparand, double fValGE, double fLT)
{
	return fComparand >= 0 ? fValGE : fLT;
}
...
#else
constexpr FORCEINLINE unsigned long LoadLittleDWord( const unsigned long *base, unsigned int dwordIndex )
	{
		return LittleDWord( base[dwordIndex] );
	}

constexpr FORCEINLINE void StoreLittleDWord( unsigned long *base, unsigned int dwordIndex, unsigned long dword )
	{
